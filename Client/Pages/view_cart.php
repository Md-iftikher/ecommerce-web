...
<body class="bg-gray-100">
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <?php if (empty($cart_items)): ?>
                <div class="bg-white rounded-lg shadow-md p-6 text-center">
                    ...
                    <a href="AllProduct.php" class="mt-4 inline-block bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-md transition duration-300">Continue Shopping</a>
                </div>
            <?php else: ?>
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <?php foreach ($cart_items as $item) { ?>
                        <div class="cart-item grid grid-cols-1 md:grid-cols-6 p-4 border-b border-gray-200 items-center" data-product-id="<?= $item['product_id'] ?>">
                            ...
                            <div class="text-center text-gray-700 mb-2 md:mb-0">
                                <div class="inline-flex items-center border border-gray-300 rounded-md quantity-controls">
                    ...
                    <div class="p-6 bg-gray-50 border-t border-gray-200">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="space-x-4">
                                <a href="AllProduct.php" class="inline-block px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-100 transition duration-300">Continue Shopping</a>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Add this inside your existing <script> tag, replacing the quantity buttons placeholder

            document.querySelectorAll('.quantity-btn').forEach(btn => {
                btn.addEventListener('click', async function() {
                    const productId = this.closest('.cart-item').dataset.productId;
                    const quantityControl = this.closest('.quantity-controls') || this.closest('.inline-flex');
                    const quantityInput = quantityControl.querySelector('.quantity-input');
                    let newQuantity = parseInt(quantityInput.textContent);
                    ...
                    try {
                        const response = await fetch('/ecommerce-frontend/Client/php/cart/update_quantity.php', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                        ...
                        // Update the subtotal
                        const price = parseFloat(this.closest('.cart-item').querySelector('.text-center.text-gray-700:not(.font-medium)').textContent.replace('$', ''));
                        const subtotalElement = this.closest('.cart-item').querySelector('.text-center.text-gray-700.font-medium');
